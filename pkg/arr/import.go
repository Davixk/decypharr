...
func (a *Arr) Import(downloadID string) (io.ReadCloser, error) {
	...
	if resp.StatusCode < http.StatusOK || resp.StatusCode >= http.StatusMultipleChoices {
		...
		return nil, fmt.Errorf("failed to import: manual-import lookup failed: %s", resp.Status)
	}
	...
	if resp == nil {
		return nil, fmt.Errorf("failed to import: command returned no response")
	}
	if resp.StatusCode < http.StatusOK || resp.StatusCode >= http.StatusMultipleChoices {
		if resp.Body != nil {
			_ = resp.Body.Close()
		}
		return nil, fmt.Errorf("failed to import: command failed: %s", resp.Status)
	}
