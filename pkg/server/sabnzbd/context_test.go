...
// newSABAuthHarness wires a bare Manager into the SABnzbd shim. Unlike
// newSABTestHarness it needs no NNTP server because the auth path never
// touches usenet — it only reads and mutates the shared arr map.
...
func TestAuthenticateDoesNotWipeAutoArrOnEmptyCredentials(t *testing.T) {
	...
	seeded := arr.NewWithOptions(category, srv.URL, "seed-token", arr.Options{Source: arr.SourceAuto})
