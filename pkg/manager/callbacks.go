...
func (m *Manager) removeTorrentPlacementsLocked(t *storage.Entry) error {
	...
	for _, placement := range t.Providers {
		...
		if placement.Provider == "usenet" {
			if m.usenet != nil {
				if t.NZBGeneration == "" {
					errs = append(errs, fmt.Errorf("cannot delete Usenet placement %s without an NZB generation", placement.ID))
				} else if err := m.usenet.DeleteForGeneration(placement.ID, t.NZBGeneration); err != nil {
...
// ownerInfohash is ignored during the scan because Switcher invokes this just
// before atomically removing that owner's reference; DeleteEntry invokes it
// after the owner row is already gone.
